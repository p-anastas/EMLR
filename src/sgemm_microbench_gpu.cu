...
///
/// \brief A cublasSgemm micro-benchmark
///
...
char* check_benchmark(short dev_id, size_t minDim, size_t Mmax, size_t Nmax, size_t Kmax, size_t M_step, size_t N_step, size_t K_step, size_t samples_max){
	...
	sprintf(filename, "%s/BenchOutputs/%s/cublasSgemm/EMLR_dev-%d_minDim-%d_max-%d-%d-%d_step-%d-%d-%d_lim-%d.log", PROJECTDIR, MACHINE, dev_id, minDim, Mmax, Nmax, Kmax, M_step, N_step, K_step, samples_max);
	...
	if (!fp) { 
		...
		fprintf(stderr,"GPU SGEMM benchmark@%s dev=%d found: minDim = %d, max dims(%d,%d,%d) with steps(%d,%d,%d) and max %d benchmarks\n", MACHINE, dev_id, minDim, Mmax, Nmax, Kmax, M_step, N_step, K_step, samples_max);
		fclose(fp);	
		exit(1); 
	}
	return filename;		  	
...
int main(const int argc, const char *argv[]) {
	...
	float *A_dev, *B_dev, *C_dev;
  	vec_alloc((void**)&A_dev, Mmax * Kmax * sizeof(float), dev_id);
  	vec_alloc((void**)&B_dev, Nmax * Kmax * sizeof(float), dev_id);
  	vec_alloc((void**)&C_dev, Mmax * Nmax * sizeof(float), dev_id);
	cudaCheckErrors();
	...
	Sinit_cuRAND(A_dev, Mmax * Kmax, 42);
	Sinit_cuRAND(B_dev, Kmax * Nmax, 42*42);
	Sinit_cuRAND(C_dev, Mmax * Nmax, 42*42*42);
	...
	for ( int itt = 0; itt <10; itt++){
		assert(CUBLAS_STATUS_SUCCESS == cublasSgemm(handle0, gpu_op_A, gpu_op_B, Mmax, Nmax, Kmax, &alpha, A_dev, ldA, B_dev, ldB, &beta, C_dev, ldC));
		...
		for (size_t N = minDim; N < Nmax + 1; N+=N_step) 
			for (size_t K = minDim; K < Kmax + 1; K+=K_step) {
					...
					for (int itt = 0; itt < ITER; itt ++) {
						cpu_timer = csecond();
						assert(CUBLAS_STATUS_SUCCESS == cublasSgemm(handle0, gpu_op_A, gpu_op_B, M, N, K, &alpha, A_dev, ldA, B_dev, ldB, &beta, C_dev, ldC));
						cudaStreamSynchronize(host_stream);
						cpu_timer  = csecond() - cpu_timer ;
